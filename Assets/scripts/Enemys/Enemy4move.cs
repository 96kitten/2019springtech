...
public class Enemy4move : MonoBehaviour {
	...
	void Update () {
		if (SceneManager.GetActiveScene ().name == "Main") {
			...
			if (search == true) {
				WMoveAnimate.SetBool("Hgobrun",true);
				speed = 5;
			...
			if (search == false) {
				WMoveAnimate.SetBool("Hgobrun",false);
				WMoveAnimate.SetBool("Hgobwalk",true);
				speed = 3;
	...
	public Vector3 GetRandomPositionOnLevel(){
		//目標地点
		//float levelSize = 55f;
		return new Vector3(Random.Range(-65, 60), 0.5f, Random.Range(65, 120));
	}
